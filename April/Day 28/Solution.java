...
class Solution {
    public int minimumEffortPath(int[][] heights) {
        ...
        PriorityQueue<int[]> pq = new PriorityQueue<int[]>((t1, t2) -> (t1[2] - t2[2]));// returns cell with minimum
                                                                                        // efforts
        ...
        while (!pq.isEmpty()) {
            ...
            for (int x[] : dir) {
                ...
                if (nrow < 0 || nrow >= n || ncol < 0 || ncol >= m)
                    continue;
                int wt = Math.max(curr_wt, Math.abs(heights[nrow][ncol] - heights[curr_row][curr_col]));// getting max
                                                                                                        // absolute
                                                                                                        // value
